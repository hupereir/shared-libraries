...
namespace SVG
{
  ...
  SvgImage::SvgImage( const QSize& size ):
    QImage( size, QImage::Format_ARGB32_Premultiplied )
  { };
  
  //________________________________________________
  SvgImage& SvgImage::render( QSvgRenderer& svg, const double& offset )
  {
    ...
    double width = SvgImage::width() + 2.0*(offset - corner_size.width() );
    double height = SvgImage::height() + 2.0*( offset - corner_size.height() );
