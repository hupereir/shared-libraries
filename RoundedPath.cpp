...
RoundedPath::RoundedPath( QPolygonF polygon, const int& border )
{
  ...
  for( int i = 1; i < polygon.size(); i++ )
  {
    ...
    if( ( next.y() == corner.y() && direction == HORIZONTAL ) || ( next.x() == corner.x() && direction == VERTICAL ) ) continue;
    
    // curve around next corner
    if( next.y() == corner.y() ) offset = QPointF( corner.x() < next.x() ? border:-border, 0 );
