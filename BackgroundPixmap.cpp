...
namespace TRANSPARENCY
{
    ...
    QPixmap BackgroundPixmap::pixmap( const QRect& rect ) const
    {
        ...
        if( backgroundPixmap_.isNull() ) return QPixmap();

        // rect is contained entirely in background
        if( backgroundPixmap_.rect().contains( rect ) )
        { return backgroundPixmap_.copy( rect ); }

        // for 1pixel pixmaps, just fill the output
        if( backgroundPixmap_.size() == QSize(1,1) )
        {
            QPixmap pixmap( rect.size() );
            pixmap.fill( backgroundPixmap_.toImage().pixel(0,0) );
            return pixmap;
        }

        // for too small pixmaps, perform tile
        // compute tile origin
        QRect background_rect( backgroundPixmap_.rect() );
        ...
        for( int x = x_origin; x <= rect.right(); x +=  background_rect.width() )
        {
            for( int y = y_origin; y <= rect.bottom(); y += background_rect.height() )
            {
                ...
                painter.drawPixmap( QPoint( x, y ), backgroundPixmap_ );
            }
        }
    ...
    void BackgroundPixmap::reload( void )
    {
        ...
        if( width && height )
        {
            ...
            */
            ...
            backgroundPixmap_ = QPixmap::fromImage( image );

        }

        #endif

        if( !backgroundPixmap_.isNull() ) emit backgroundChanged();
